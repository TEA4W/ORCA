        ...
        async function fetchEquipmentData() {
            try {
                ...
                // Optionally, display an error to the user or use fallback data
                alert("Erro ao carregar dados dos equipamentos. Algumas funcionalidades podem não estar disponíveis.");
            }
        }
        ...
        async function fetchTransportationFees() {
            try {
                ...
                alert("Erro ao carregar dados das taxas de transporte. Algumas funcionalidades podem não estar disponíveis.");
            }
        }
        ...
        async function fetchPaymentConditions() {
            try {
                ...
                alert("Erro ao carregar dados das condições de pagamento.");
            }
        }
        ...
        async function fetchPaymentMethods() {
            try {
                ...
                alert("Erro ao carregar dados dos métodos de pagamento.");
            }
        }
        ...
        function addItem() {
            ...
            removeButton.innerHTML = '<i class="fas fa-trash-alt"></i>'; // Use trash icon
            ...
            itemDiv.appendChild(nameFieldContainer);
            ...
            itemEquipTimingDiv.style.display = 'none'; // Initially hidden 
            ...
            dateTimeTitle.textContent = 'Datas e Horários';
            ...
            // Hide the section by default, then let visibility be updated.
            dateTimeTitle.style.display = 'none';
            itemOptionsContainer.style.display = 'none';
        ...
        function update() {
            ...
            if (showPricesWithoutIva) {
                ...
                if (globalDiscountPercent > 0) {
                    ...
                    appendSummaryLine(`Desconto Global (${globalDiscountPercent.toFixed(1)}% s/ €${totalEquipBaseNet.toFixed(2)} da componente Equip. Base)`, -globalDiscountAmount, 'discount-line');
                }
                ...
                if (transportFeeIvaInclusive > 0) {
                 ...
                 if (globalDiscountPercent > 0) {
                    ...
                    appendSummaryLine('SUBTOTAL (Antes Desconto Global)', subtotalAllBeforeDiscount);
                    appendSummaryLine(`Desconto Global (${globalDiscountPercent.toFixed(1)}% s/ €${(totalEquipBaseNet * (1+IVA_RATE)).toFixed(2)} da componente Equip. Base)`, -(globalDiscountAmount * (1 + IVA_RATE)), 'discount-line');
                }
...
.no-print { display: none !important; } 
                ...
                for (let i = 0; i < idx; i++) {
                    ...
                    if (details) {
                        ...
                        if (details.image_url) {
                            ...
                            htmlContent += `<img src="${imageUrl}" alt="Imagem de ${itemName}" onerror="this.style.display='none'; this.parentElement.innerHTML += '<p style=\\'color:red;\\'>ERRO: Imagem não encontrada em ${imageUrl}</p>';">`;
                        } else {
                ...
                if (itemDiv) {
                    ...
                    if (nameInput && nameInput.value.trim() !== "") {
                        items.push({
                            ...
                            basePriceDiscountToggle: document.getElementById(`basePriceDiscountToggle${i}`).checked,
                            basePriceDiscountPercentage: document.getElementById(`basePriceDiscountPercentage${i}`).value,
        ...
        function applyQuoteDataToForm(data) {
            ...
            if (data.items && data.items.length > 0) {
                data.items.forEach(itemData => {
                    ...
                    document.getElementById(`basePriceDiscountToggle${currentIdx}`).checked = itemData.basePriceDiscountToggle;
                    document.getElementById(`basePriceDiscountPercentage${currentIdx}`).value = itemData.basePriceDiscountPercentage;
                    ...
                    document.getElementById(`basePriceDiscountToggle${currentIdx}`).dispatchEvent(new Event('change'));
        ...
        function checkPasswordAndShowApp() {
            ...
            if (savedPassword === CORRECT_PASSWORD) {
                ...
                initializeApp(); // Initialize the app only after correct password entry
        ...
        function displayEquipmentList(searchTerm = '') {
            ...
            filteredEquipment.forEach(equipName => {
                ...
                // Use the processed image_url and encode it for the URL
                    ...
                    <div class="equipment-card-content">
                        ...
                        <p class="equipment-card-description">${equipment.description.replace(/\\n/g, '<br>')}</p>
                         <div class="equipment-card-actions">
                            <button class="edit-btn" onclick="handleEditEquipment('${equipName}')"><i class="fas fa-pencil-alt"></i></button>
                            <button class="delete-btn" onclick="handleDeleteEquipment('${equipName}')"><i class="fas fa-trash-alt"></i></button>
                        </div>
                    </div>
                `;
        ...
        async function handleDeleteEquipment(equipmentName) {
            ...
            if (confirm(`Tem a certeza que quer apagar o equipamento "${equipmentName}"?`)) {
                ...
                if (currentlyEditingEquipmentName === equipmentName) {
                    ...
                    document.getElementById('addEquipmentForm').reset();
                    document.getElementById('imagePreview').src = 'imagens/logo-orca.png';
                    document.getElementById('addEquipmentForm').querySelector('button[type="submit"]').textContent = 'Adicionar Equipamento';
                }
                
                displayEquipmentList(document.getElementById('searchEquipmentInput').value); 
                populateEquipmentDropdowns();
                await handleSaveChangesToJson();
            }
        }

        async function handleSaveChangesToJson() {
            try {
                const response = await fetch('api.php?endpoint=save_equipment', {
                    method: 'POST',
                    headers: {
                ...
                const result = await response.json();
                console.log('Equipment changes saved successfully');
                return result;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (checkPasswordAndShowApp()) {
                ...
                // Show password screen if no valid saved password
